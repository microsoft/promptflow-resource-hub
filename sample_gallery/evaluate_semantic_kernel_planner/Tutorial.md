...
In subsequent steps, we will demonstrate the batch test and evaluation based on the [sample flow](./sk_planner_flow). If you wish to use your own flow, converted using the init command above, please ensure it has the same input and output structure as the sample flow. Also, ensure it uses the same connection (further details on connection will be provided below).
...
Click on the "output" url to open the output.jsonl file, which contains the output of the flow run.
